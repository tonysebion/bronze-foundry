"""Script to verify S3 config parsing and file reading.

This mirrors `tests/test_s3_pipeline.py` moved to scripts/ to avoid pytest discovery.
"""
...
def config_loading(config_path: Path):
    """Load config and print storage details."""
    print("=" * 80)
...
def s3_source_reading(dataset, env_config):
    """Test reading source files from S3 and print sample lines."""
    print("\n" + "=" * 80)
...
def s3_write_simulation(dataset, env_config):
    """Simulate and print S3 output paths for Bronze and Silver."""
    print("\n" + "=" * 80)
...
def main():
    ...
    print("\n" + "=" * 80)
    print("S3 Configuration Check: New Storage Section Format")
    print("=" * 80)
    print(f"Config: {config_path}")

    dataset, env_config = config_loading(config_path)

    if not dataset or not env_config:
        print("\n[ERROR] Config loading failed")
        return 1

    read_ok = s3_source_reading(dataset, env_config)

    if not read_ok:
        print("\n[ERROR] S3 source reading failed")
        return 1

    write_ok = s3_write_simulation(dataset, env_config)

    if write_ok:
        print("\n" + "=" * 80)
        print("[OK] All S3 configuration checks PASSED!")
        print("=" * 80)

...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Silver samples with hierarchical structure matching Bronze")
    parser.add_argument(
        "--formats",
...
def _ensure_bronze_samples_present() -> None:
    ...
    if SOURCE_SAMPLE_ROOT.exists():
        ...
        return
    print("[INFO] Bronze samples missing; " "source samples absent so running scripts/generate_sample_data.py")
...
def _render_polybase_ddl(
    ...
    ddl: Dict[str, Any] = {}
    ddl["external_data_source"] = _split_lines(_external_data_source_sql(setup.external_data_source))
    ddl["external_file_format"] = _split_lines(_external_file_format_sql(setup.external_file_format))
    tables: List[Dict[str, Any]] = []
    for ext_table in setup.external_tables:
        tables.append(
            {
                "table_name": ext_table.table_name,
                "ddl": _split_lines(_external_table_sql(ext_table, dataset, artifact_relative, setup)),
            }
        )
...
def _external_table_sql(
    ...
    data_source_name = (
        f"[{setup.external_data_source.name}]" if setup.external_data_source else "[undefined_data_source]"
    )
    file_format_name = (
        f"[{setup.external_file_format.name}]" if setup.external_file_format else "[undefined_file_format]"
    )
...
def _generate_for_partition(
    ...
    silver_base = TEMP_SILVER_SAMPLE_ROOT / f"sample={pattern_id}" / f"silver_model={config.silver_model}"
...
def main() -> None:
    ...
    if not partitions:
        ...
        partitions = list(_find_bronze_partitions())
        if not partitions:
            raise RuntimeError("No Bronze partitions found even after generating sample data.")
    ...
    for idx, partition in enumerate(partitions):
        ...
        if not configs:
            print(f"[WARN] No pattern configs found for pattern '{partition['pattern']}' - skipping")
            continue

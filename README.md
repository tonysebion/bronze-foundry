...
- **Silver Join**: Join two existing Silver assets with `silver_join.py`, including checkpoints (`progress.json`) and richer metadata that trace back to the Bronze inputs. See `docs/SILVER_JOIN.md` and `docs/examples/configs/silver_join_example.yaml` for configuration patterns.
